...
namespace WindowsFormsApp3
{
    public partial class Form1 : Form
    {
        ...
        //Повідомлення про імениника
        private void pictureBox2_Click(object sender, EventArgs e)
        {
            OleDbCommand command = myConnection.CreateCommand();

            command.CommandText = "SELECT data, name FROM Workers";
            ...
            while (reader1.Read())
            {
                if (dateTimePicker1.Text == reader1["data"].ToString())
                {
                    MessageBox.Show(("Сьогодні День Народження у ") + reader1["name"]);
                }
            }
